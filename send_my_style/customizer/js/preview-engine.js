...
const PreviewEngine = (() => {
  ...
  function setUploadImage(groupId, img) {
    if (img) uploadImgCache.set(groupId, img);
    else uploadImgCache.delete(groupId);
  }
  function clearUploadCache() {
    uploadImgCache.clear();
  }
  ...
  const Prototypes = {
    shirt(w, h) {
      ...
      if (pattern && pattern !== "plain") {
        ctx.save();
        ctx.beginPath();
        ctx.rect(w * 0.24, h * 0.26, w * 0.52, h * 0.6);
        ctx.clip();
        ctx.strokeStyle = "rgba(72,31,41,0.18)";
        ctx.lineWidth = 1;
        if (pattern === "stripes") {
          ...
          for (let x = w * 0.24; x < w * 0.76; x += 18) {
            ctx.beginPath();
            ctx.moveTo(x, h * 0.26);
            ctx.lineTo(x, h * 0.86);
            ctx.stroke();
          }
          for (let y = h * 0.26; y < h * 0.86; y += 18) {
            ctx.beginPath();
            ctx.moveTo(w * 0.24, y);
            ctx.lineTo(w * 0.76, y);
            ctx.stroke();
      ...
      if (collar === "mandarin") {
        ctx.beginPath();
        ctx.moveTo(w * 0.4, h * 0.24);
        ctx.lineTo(cx, h * 0.3);
        ctx.lineTo(w * 0.6, h * 0.24);
        ...
        ctx.lineTo(w * (0.5 - spread), h * 0.18);
      ...
      ctx.fillStyle = fill;
      ctx.strokeStyle = STROKE;
      ctx.fillRect(w * 0.12, h * 0.5, w * 0.08, cuffH);
      ctx.strokeRect(w * 0.12, h * 0.5, w * 0.08, cuffH);
      ctx.fillRect(w * 0.8, h * 0.5, w * 0.08, cuffH);
      ctx.strokeRect(w * 0.8, h * 0.5, w * 0.08, cuffH);
      // buttons
    ...
    tshirt(w, h) {
      ...
      if (neck === "vneck") {
        ...
        ctx.moveTo(w * 0.42, h * 0.2);
        ctx.quadraticCurveTo(w * 0.5, h * 0.28, w * 0.58, h * 0.2);
      }
      ctx.strokeStyle = STROKE;
      ctx.lineWidth = 3;
      ctx.stroke();
    ...
    kurti(w, h) {
      ...
      if (vneck) {
        ...
        ctx.moveTo(w * 0.44, h * 0.18);
        ctx.quadraticCurveTo(w * 0.5, h * 0.26, w * 0.56, h * 0.18);
      }
      ctx.lineWidth = 3;
      ctx.stroke();
      if (chosen("embroidery") === "floral") {
        ctx.fillStyle = "#B98";
        flower(w * 0.5, h * 0.5);
      }
    },
    ...
    dress(w, h) {
      ...
      if (square) {
        ...
        ctx.moveTo(w * 0.44, h * 0.18);
        ctx.quadraticCurveTo(w * 0.5, h * 0.27, w * 0.56, h * 0.18);
      }
      ctx.lineWidth = 3;
      ctx.stroke();
    },
    ...
    shoe(w, h) {
      ...
      ctx.quadraticCurveTo(w * 0.55, h * 0.46, w * 0.72, h * 0.6);
      ctx.lineTo(w * 0.68, h * 0.64);
      ctx.quadraticCurveTo(w * 0.54, h * 0.54, w * 0.34, h * 0.64);
      ctx.closePath();
      ctx.fill();
      // laces
      ...
      ctx.lineWidth = 3;
      for (let i = 0; i < 3; i++) {
        const y = h * (0.46 + i * 0.04);
        ctx.beginPath();
        ctx.moveTo(w * 0.4, y);
        ctx.lineTo(w * 0.56, y + h * 0.02);
        ctx.stroke();
      }
      // sole
    ...
    bag(w, h) {
      ...
      if (longStrap) {
        ...
        ctx.quadraticCurveTo(w * 0.5, h * 0.08, w * 0.68, h * 0.42);
      } else {
        ctx.moveTo(w * 0.38, h * 0.42);
        ctx.quadraticCurveTo(w * 0.5, h * 0.26, w * 0.62, h * 0.42);
      }
      ctx.stroke();
      // hardware clasp
    ...
    cap(w, h) {
      ...
      ctx.fill();
      ctx.stroke();
      if (chosen("patch") === "minimal") {
        ctx.fillStyle = "#FBF7F4";
        ctx.fillRect(w * 0.44, h * 0.4, w * 0.12, h * 0.06);
      }
    },
    ...
    necklace(w, h) {
      ...
      if (heart) {
        ...
        ctx.beginPath();
        ctx.arc(w * 0.5, h * 0.56, 12, 0, Math.PI * 2);
        ctx.fill();
      }
    },
  };
  ...
  function flower(x, y) {
    for (let i = 0; i < 6; i++) {
      const a = (Math.PI / 3) * i;
      ctx.beginPath();
      ctx.ellipse(x + Math.cos(a) * 10, y + Math.sin(a) * 10, 6, 3, a, 0, Math.PI * 2);
      ctx.fill();
    }
  }
  ...
  function drawUpload(w, h, group) {
    ...
    const ax = w * (area.x / 100),
      ay = h * (area.y / 100),
      aw = w * (area.width / 100),
      ah = h * (area.height / 100);
    ...
    const dw = img.width * bs * t.scale,
      dh = img.height * bs * t.scale;
    ctx.translate(ax + aw / 2 + t.dx, ay + ah / 2 + t.dy);
    ctx.rotate(((t.rotation || 0) * Math.PI) / 180);
    ctx.drawImage(img, -dw / 2, -dh / 2, dw, dh);
    ctx.restore();
    ctx.save();
    ctx.strokeStyle = "rgba(196,92,114,0.5)";
    ctx.setLineDash([6, 4]);
